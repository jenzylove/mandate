...
// A listing is live when it came from the onchain registry rather than the
// seed file. Every caveat in this file keys off this rather than assuming.
const isLive = (agent: Agent) => agent.source !== "seed";
...
export function HomeExperience({ outcomes, agents }: { outcomes: Outcome[]; agents: Agent[] }) {
  ...
  return (
    <main className="mh-home mh-wrap">
      ...
      {selectedAgent && <div className="mh-modal-backdrop" role="presentation" onMouseDown={() => setSelectedAgent(null)}><section className="mh-agent-modal" role="dialog" aria-modal="true" aria-labelledby="agent-modal-title" onMouseDown={(event) => event.stopPropagation()}><button aria-label="Close details" onClick={() => setSelectedAgent(null)}>×</button><p className="mh-eyebrow">AGENT DETAILS · {isLive(selectedAgent) ? "LIVE ONCHAIN AGENT" : "DEMO LISTING"}</p><div className="mh-modal-title"><h2 id="agent-modal-title">{selectedAgent.name}</h2><span className={`mh-availability ${selectedAgent.status}`}>● {selectedAgent.status}{isLive(selectedAgent) ? "" : " · demo"}</span></div><p>{selectedAgent.description}</p><dl><div><dt>{isLive(selectedAgent) ? "Price quoted by the agent" : "Example fee"}</dt><dd>{selectedAgent.pricing}</dd></div><div><dt>Protocols &amp; assets</dt><dd>{selectedAgent.protocols.join(" / ")} · {selectedAgent.assets.join(" / ")}</dd></div><div><dt>Supported control</dt><dd>{selectedAgent.supportedControlModes.map((mode) => controlCopy[mode]).join(" · ")}</dd></div></dl><div className="mh-modal-evidence"><strong>Evidence · {selectedAgent.evidence.provenance} · {selectedAgent.evidence.windowDays} days</strong>{selectedAgent.evidence.metrics.map((metric) => <p key={metric.label}><span>{metric.label}</span><b>{metric.value}<small>{metric.provenance}</small></b></p>)}<small>{isLive(selectedAgent) ? "Read from the agent's own registration and endpoint. Not a projected return." : "Seeded sample, not a live result or projected return."}</small></div><div className="mh-modal-note">{isLive(selectedAgent) ? "Registered onchain and answering. Open the profile to see its identity, transports and evidence, and to hire it." : "This is seeded placeholder inventory. It has no onchain identity and cannot be hired."}</div><Link className="mh-button mh-purple" href={`/agents/${selectedAgent.id}`}>Open agent profile ↗</Link></section></div>}
    </main>
  );
}
